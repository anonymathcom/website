...
test.describe
  .parallel('Open theme menu in home page and click dark mode to set page to corresponding dark theme', () => {
...
test.describe
  .parallel('Open theme menu in home page and click light mode to set page to corresponding light theme', () => {
  locales.forEach(function (locale: string) {
    test(locale.toLocaleUpperCase(), async ({ page }) => {
      let url = process.env.E2E_START_URL ?? 'http://localhost:3000';
      if (defaultLocale != locale) {
